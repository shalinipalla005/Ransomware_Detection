"""
RansomWall: Dataset Augmentation
==================================
Generates synthetic feature vectors to augment a small real dataset.

Produces:
  - N ransomware-like rows  (label=1)
  - N benign-like rows      (label=0)

Feature distributions derived from paper §IV-A and §V-B analysis.
Real samples in dataset.csv are preserved and augmented, not replaced.

Usage:
  python augment_dataset.py [--input dataset.csv] [--output dataset.csv] [--n 200]
"""
...
# ── Ransomware feature distribution (paper §V-B sandbox stats) ───────────────
def _ransomware_row(idx: int) -> dict:
    row = {
        # Trap features — all commonly triggered by ransomware
        ...
        # Dynamic features — heavy file churn
        ...
        # Static features — typically unsigned, packed
...
# ── Benign feature distribution ───────────────────────────────────────────────
def _benign_row(idx: int) -> dict:
    row = {
        # Trap features — benign processes don't touch honey files
        "honey_file_write":       _rb(0.02),
        "honey_file_rename":      _rb(0.01),
        "honey_file_delete":      _rb(0.01),
        "honey_dir_modified":     _rb(0.05),
        "crypto_api_usage":       _rb(0.15),   # e.g. browser TLS
        "safe_mode_disabled":     0,
        "vss_deletion":           0,
        "registry_persistence":   _rb(0.08),   # e.g. legitimate installers
        "entropy_spike":          _rb(0.05),
        # Dynamic features — light, normal usage
        ...
        # Static features — typically signed, not packed
...
def augment(input_csv: str, output_csv: str, n: int):
    ...
    with open(output_csv, "w", newline="", encoding="utf-8") as f:
        ...
        writer.writeheader()
        for row in all_rows:
            # Ensure all columns present (fill missing with 0)
